...
Classes:
    Address: Represents a postal address with number, street, city, state,
        zip code, and country ISO code.
    Letting: Represents a property letting with a title and associated address.
...
class Address(models.Model):
    ...
    Fields:
        number (PositiveIntegerField): Street number, must be between 1 and 9999.
        street (CharField): Street name, up to 64 characters.
        city (CharField): City name, up to 64 characters.
        state (CharField): State code (2 characters), must be exactly 2 characters.
...
class Letting(models.Model):
    ...
    Fields:
        ...
        address (OneToOneField): A one-to-one relationship to an Address instance,
            representing the location of the letting. Deleting the address will
            also delete the letting.
